...
def train(train_loader, model, optimizer: AdamW, args):
    ...
    for batch in train_loader:
        ...
        # Predict masks
        pred_masks = model(
            sam_imgs_tensor, beit_imgs_tensor, clip_embeddings, clap_embeddings, original_size_list
        )
        # for S4 dataset
        # if T_i == 5 * T_m:
...
def evaluate(test_loader, model, args):
    ...
    eval_metrics = {"miou": avg_meter_miou.pop("miou").item(), "F_score": avg_meter_F.pop("F_score")}
...
def main(args):
    ...
    name = f"{args.model_name}_{args.projector_type}{'_adapter' if args.use_adapter else ''}_{args.evf_version}_{args.dataset}"

    if args.wandb:
        if args.dataset == "s4":
            project_name = "avsbench_s4"
        else:
            project_name = "avsbench"
    ...
    print(
        f"Training {args.model_name} on {args.dataset} with {args.evf_version} for {args.max_epochs} epochs"
    )
    ...
    if args.wandb:
        wandb.log({"test_miou": test_result["miou"], "test_F_score": test_result["F_score"]})
