...
class CLAPWrapper():
    ...
    def load_audio_into_tensors(self, audio_path, resample=True):
        r"""Loads audio file and returns raw audio."""
        # Randomly sample a segment of audio_duration from the clip or pad to match duration
        audio_time_series, sample_rate = self.read_audio(audio_path, resample=resample)
        ...
        # Total number of full seconds in the audio
        total_seconds = 5
        
        # Split the audio into 1-second segments while retaining both channels
        audio_segments = []
        total_samples = audio_time_series.shape[1]

        for i in range(total_seconds):
            start_index = i * samples_per_second
            end_index = (i + 1) * samples_per_second

            if end_index > total_samples:
                segment = audio_time_series[:, start_index:total_samples]
                padding_length = samples_per_second - segment.shape[1]
                # padd with zeros
                segment = torch.cat((segment, torch.zeros((audio_time_series.shape[0], padding_length))), dim=1)

                print(f"Padding audio segment with {padding_length} zeros")
                
            else:
                segment = audio_time_series[:, start_index:end_index]  # Get both channels
            
            # Flatten the segment to combine both channels into one array
            flattened_segment = segment.reshape(-1)
            audio_segments.append(torch.FloatTensor(flattened_segment))
    ...
    def get_audio_embeddings_single(self, path):
        preprocessed_audio = self.preprocess_a_audio(path)
        return self._get_audio_embeddings(preprocessed_audio)
    ...
    def preprocess_a_audio(self, audio_file, resample = True):
        r"""Load list of audio files and return raw audio"""
        audio_tensors = self.load_audio_into_tensors(audio_file)
        if self.use_cuda and torch.cuda.is_available():
            audio_tensors = [audio_tensor.reshape(1, -1).cuda() for audio_tensor in audio_tensors]
        else:
            audio_tensors = [audio_tensor.reshape(1, -1) for audio_tensor in audio_tensors]
        return self.default_collate(audio_tensors)

    def generate_caption_single(self, path, beam_size: int = 5, entry_length=67, temperature=1.):
        r"""Generate audio captions for each audio recording in a batch"""
        # audio_tensors = self.preprocess_audio(audio_files, resample)
        audio_tensors = self.preprocess_a_audio(path)
