...
class EvfSam2Model(PreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.train_prompt_encoder = kwargs.get("train_prompt_encoder",True)
        print(f"train_mask_decoder: {self.train_mask_decoder}")
        print(f"train_prompt_encoder: {self.train_prompt_encoder}")
    ...
    def initialize_evf_modules(self, config):
        ...
        # train cross attn
        for name, param in self.visual_model.image_encoder.trunk.named_parameters():
            if "cross_attn" in name:
                param.requires_grad = True

        if self.train_mask_decoder:
            print("train mask decoder")
    ...
    def get_beit3_features(self,images_evf,text_embeddings):
        BT, C, E = text_embeddings.shape
        size = (BT, C)
        text_padding_position = torch.zeros(size).to(text_embeddings.device)
        output = self.mm_extractor.beit3.forward_text_embedding(visual_tokens=images_evf, text_embeddings=text_embeddings, text_padding_position=text_padding_position)
    ...
    def get_sam_encoder_features(self,images, prompt_features):
        backbone_out = self.visual_model.forward_image(images, prompt_features)
        return backbone_out

    def inference_text_embeddings(
            self,
            images,
            backbone_out,
            beit3_features,
        ...
        # insert feat to image encoder
        # backbone_out = self.visual_model.forward_image(images, prompt_features)
        
        # print(f"vision feature : {backbone_out['vision_features'].shape}")
        # dict_keys(['vision_features', 'vision_pos_enc', 'backbone_fpn'])
        _, image_embeddings, _, _ = self.visual_model._prepare_backbone_features(backbone_out)
        image_embeddings = [_.to(images.dtype) for _ in image_embeddings]
        ...
        # prompt encoder 
        feat = self.text_hidden_fcs[0](beit3_features)
        (
            sparse_embeddings,
            dense_embeddings,
        ) = self.visual_model.sam_prompt_encoder(
        ...
        if dense_prompt_embeddings is not None:
            dense_embeddings = dense_prompt_embeddings
